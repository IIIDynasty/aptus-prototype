...
// ── Hausa AI Translation Endpoint ──────────────────────────────────────────
app.post('/api/jobs/translate', async (req, res) => {
  try {
    const { title, description, responsibilities, qualifications, perks, niceToHave } = req.body;

    const toList = (arr) => Array.isArray(arr) && arr.length > 0
      ? arr.map((item, i) => `${i + 1}. ${item}`).join('\n')
      : 'N/A';

    const prompt = `Translate the following job posting content into professional Nigerian Hausa language.
Preserve the exact structure and return ONLY a valid JSON object with no extra text.

English Content:
- Title: ${title || ''}
- Description: ${description || ''}
- Responsibilities:\n${toList(responsibilities)}
- Qualifications:\n${toList(qualifications)}
- Perks/Benefits:\n${toList(perks)}
- Nice to Have:\n${toList(niceToHave)}

Return this exact JSON structure with the Hausa translations:
{
  "title": "...",
  "description": "...",
  "responsibilities": ["...", "..."],
  ...
  "niceToHave": ["...", "..."]
    ...
    const chatCompletion = await groq.chat.completions.create({
      ...
      messages: [
        {
          role: 'system',
          content: 'You are a professional translator specializing in Nigerian Hausa language for business and employment contexts. Return ONLY valid JSON with no markdown or extra explanation.'
        },
    ...
    console.log('✅ Hausa translation generated by Groq');
    res.json({ success: true, translation });

  } catch (error) {
    console.error('❌ Hausa translation failed:', error.message);
